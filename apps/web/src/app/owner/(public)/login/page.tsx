...
async function doLogin(formData: FormData) {
  ...
  const res = await fetch(`${BASE_URL}/api/owner/auth/login`, {
    method: "POST",
  ...
  if (!res.ok || !json || !json.ok) {
    // después lo hacemos lindo con errores visibles
    return;
  }
...
export default function OwnerLoginPage() {
  return (
    <Card title="Iniciar sesión (Owner)" subtitle="Entrá con tu email y contraseña.">
      <form action={doLogin} style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ marginTop: 10, fontSize: 13, opacity: 0.8 }}>
          ¿No tenés cuenta? <Link href="/owner/register">Crear cuenta</Link>
        </div>
      </form>
    </Card>
  );
